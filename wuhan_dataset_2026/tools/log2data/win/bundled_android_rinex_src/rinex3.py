...
def write_obs(mdict, obslist, flag=0):
    ...
    try:
        ...
        return ''
    whole_seconds = int(epoch_sec // 1)
    fractional_digits = int(round((epoch_sec - whole_seconds) * 1e7))
    ...
    if not rounded_to_next_second and fractional_digits > 5000000 and epoch.microsecond < 500000:
        epoch_for_format -= datetime.timedelta(seconds=1)
    res = epoch_for_format.strftime("> %Y %m %d %H %M %S.") + '{0:07d}'.format(fractional_digits)
