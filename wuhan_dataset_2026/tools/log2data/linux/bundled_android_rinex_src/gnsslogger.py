...
def filter_obs(measurement, model):
    """
    Keep Android Raw measurements for RINEX export.

    This converter is used as a raw data extractor. Quality control belongs to
    downstream preprocessing, so code-lock/TOW/CN0/multipath thresholds must not
    silently remove measurements here.
    """
    if measurement['ConstellationType'] == CONSTELLATION_UNKNOWN:
...
def process(measurement, model, fix_bias=True, timeadj=1e-7, pseudorange_bias=0.0, 
    ...
    # the transmitted time. Do not suppress high-uncertainty raw measurements;
    # downstream processing can decide how to filter them.
    ...
    # Process the accumulated delta range (i.e. carrier phase). This needs to be
    # translated from meters to cycles. Keep high-uncertainty or discontinuous
    # measurements in the raw export and mark them via slip/STD instead of
    # zeroing them at extraction time.
    ...
    if range != 0.0:
        ...
        Pstd = 0 if PRU <= 0 else max(0,min(9, round(math.log2(PRU * 100)) - 5))
        if toff != 0.0:
